...
class ERP
{
    ...
    public function getListaFattureDaEmettere($filtro_cliente = null)
    {
        $lista = array();
        $con = DBUtils::getConnection();
        $sql = "SELECT * FROM fatture_da_emettere";
        $res = mysql_query($sql);
        while ($row = mysql_fetch_object($res))
        {
            if ($filtro_cliente)
            ...
            $lista[] = array('id_cliente'=>$row->id_cliente,
    ...
    public function getFatturato($periodo = null)
    {
        ...
        {
            $sql .=" AND data BETWEEN '".$periodo[ERP::FILTRO_PERIODO_DAL]."' AND '".$periodo[ERP::FILTRO_PERIODO_AL]."'";
        }
        else
        {
            if (array_key_exists(ERP::FILTRO_PERIODO_DAL, $periodo))
            {
                $sql .= " AND data>='".$periodo[ERP::FILTRO_PERIODO_DAL]."'";
            }
            if (array_key_exists(ERP::FILTRO_PERIODO_AL, $periodo))
            {
                $sql .= " AND data<='".$periodo[ERP::FILTRO_PERIODO_AL]."'";
            }
        }
